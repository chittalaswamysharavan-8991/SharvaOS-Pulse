...
function hasD1Binding(values) {
  const binding = values?.[D1_BINDING_ENV];
  return Boolean(binding && typeof binding === "object" && typeof binding.prepare === "function");
}

export function resolvePulseRuntimeValues(values = {}) {
  const ownerValue = text(values, OWNER_ENV).toLowerCase();
  if (ownerValue === "d1") {
    if (!hasD1Binding(values)) {
      return blocked(
        "D1 rollback blocked: Cloudflare D1 binding `DB` is unavailable. Keep Supabase active until this exact release is deployed on a D1-capable runtime.",
        "d1",
      );
    }
    return {
